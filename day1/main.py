...
def read_sequence(file_path: str) -> list[int]:
    """Read a sequence of digits from a file."""

    with open(file_path, encoding="utf-8") as file:
        return [int(digit) for digit in file.read().strip()]


def sum_matching_pairs(
    sequence: list[int],
    next_digit: Callable[[list[int], int], int],
) -> int:
    """Sum the digits that match the next digit in the circular sequence."""

    return sum(
        digit for i, digit in enumerate(sequence) if digit == next_digit(sequence, i)
    )


def get_following_digit(sequence: list[int], index: int) -> int:
    """Retrieve the next digit after the given index in the circular sequence."""

    return sequence[(index + 1) % len(sequence)]


def get_halfway_digit(sequence: list[int], index: int) -> int:
    """Retrieve the digit halfway around the circular sequence from the given index."""
