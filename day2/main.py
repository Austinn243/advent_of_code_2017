...
def summarize_spreadsheet(
    ...
    process_row: Callable[[list[int]], int],
) -> int:
...
def evenly_divisible_quotient(row: list[int]) -> int:
    """Divide the only two numbers in a row that evenly divide each other."""
...
def main() -> None:
    ...
    sum_evenly_divisible_quotients = partial(
        summarize_spreadsheet,
        process_row=evenly_divisible_quotient,
    )
